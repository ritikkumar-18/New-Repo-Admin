...
//                     <motion.div
//                       key={applicant.id}
//                       className="bg-gray-800 rounded-xl overflow-hidden border border-gray-700 hover:border-purple-500 transition-all duration-300 shadow-lg flex flex-col"
//                       initial={{ opacity: 0 }}
//                       animate={{ opacity: 1 }}
//                       transition={{ duration: 0.5 }}
//                       whileHover={{ y: -5, boxShadow: "0 10px 25px -5px rgba(124, 58, 237, 0.1)" }}
...
//                         <div className="h-24 bg-gradient-to-r from-purple-900/60 to-indigo-900/60"></div>
...
//                         <p className="text-purple-400 font-medium mb-4">{applicant.currentRole}</p>

//                         <div className="space-y-3 mb-6">
//                           <div className="flex items-center text-sm text-gray-300">
//                             <Briefcase size={16} className="mr-3 text-purple-400 flex-shrink-0" />
...
//                           <div className="flex flex-wrap gap-2">
//                             {applicant.skills.slice(0, 3).map((skill, index) => (
//                               <span key={index} className="px-2.5 py-1 bg-gray-700 text-xs rounded-full text-gray-300">
//                                 {skill}
//                               </span>
//                             ))}
//                             {applicant.skills.length > 3 && (
//                               <span className="px-2.5 py-1 bg-gray-700 text-xs rounded-full text-gray-300">
//                                 +{applicant.skills.length - 3}
//                               </span>
//                             )}
//                           </div>
...
//                           <div className="flex items-center">
//                             <p className="text-sm text-gray-400 mr-2">Rating:</p>
//                             {renderStarRating(applicant.rating)}
//                             <span className="ml-2 text-gray-300 text-sm">{applicant.rating.toFixed(1)}</span>
//                           </div>
//                           <div className="flex items-center gap-2">
//                             <span className="text-sm text-gray-400">Experience:</span>
//                             <span className="text-sm font-medium text-white">{applicant.experience}</span>
//                           </div>
//                         </div>
//                       </div>
...
//                         <button
//                           onClick={() => handleContactApplicant(applicant)}
//                           className="flex items-center justify-center py-2 px-4 bg-green-600/20 text-green-400 hover:bg-green-600/30 rounded-lg transition-colors"
//                         >
//                           <Mail size={16} className="mr-2" /> Contact
//                         </button>
//                         <button
//                           onClick={() => handleShortlistApplicant(applicant)}
//                           className={`col-span-2 flex items-center justify-center py-2 px-4 rounded-lg transition-colors ${
//                             applicant.status === "Shortlisted"
...
//                           {applicant.status === "Shortlisted" ? (
//                             <>
//                               <CheckCircle size={16} className="mr-2" /> Remove from Shortlist
//                             </>
//                           ) : (
//                             <>
//                               <Plus size={16} className="mr-2" /> Add to Shortlist
//                             </>
//                           )}
//                         </button>
//                       </div>
...
//                       className="px-4 py-3 w-full border border-gray-700 rounded-md bg-gray-800 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
//                       required
//                       defaultValue={`Regarding your application for ${selectedJobApplications?.title || "our position"}`}
//                     />
//                   </div>
...
//                       className="px-4 py-3 w-full border border-gray-700 rounded-md bg-gray-800 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent min-h-[150px] resize-y"
//                       required
//                       defaultValue={`Hi ${selectedApplicant.name},\n\nThank you for your application for the ${selectedJobApplications?.title || "position"} at our company. We've reviewed your profile and would like to discuss your application further.\n\nBest regards,\nRecruitment Team`}
//                     />
//                   </div>
...
//                       <button
//                         type="button"
//                         onClick={() => toast.success(`Email draft prepared for ${selectedApplicant.email}`)}
//                         className="flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600/30 to-blue-700/30 text-blue-300 border border-blue-600/50 py-2 px-4 rounded-md hover:from-blue-600/40 hover:to-blue-700/40"
//                       >
//                         <Mail size={16} /> Email
//                       </button>
//                       <button
//                         type="button"
//                         onClick={() => toast.success(`Calling ${selectedApplicant.phone}...`)}
//                         className="flex items-center justify-center gap-2 bg-gradient-to-r from-green-600/30 to-green-700/30 text-green-300 border border-green-600/50 py-2 px-4 rounded-md hover:from-green-600/40 hover:to-green-700/40"
//                       >
//                         <Phone size={16} /> Call
//                       </button>
//                       <button
//                         type="button"
//                         onClick={() => toast.success(`SMS draft prepared for ${selectedApplicant.phone}`)}
//                         className="flex items-center justify-center gap-2 bg-gradient-to-r from-yellow-600/30 to-yellow-700/30 text-yellow-300 border border-yellow-600/50 py-2 px-4 rounded-md hover:from-yellow-600/40 hover:to-yellow-700/40"
//                       >
//                         <MessageSquare size={16} /> SMS
//                       </button>
//                     </div>
...
// import { motion, AnimatePresence } from "framer-motion";
// import { toast } from "react-hot-toast";
// import { Search, X, Trash2, Calendar, DollarSign, Users, Share2, Eye, ToggleLeft, ToggleRight, FileText, Upload, ChevronLeft, ChevronRight, CheckCircle, Plus, Mail, Briefcase, MapPin, Phone, Star, Edit, Clock, User, GraduationCap, Award, Download, ExternalLink, MessageSquare, Paperclip, Link2, Linkedin, Github, Lock } from 'lucide-react';
// import Header from "../components/Common/Header";

// const Jobopening = () => {
//   // Expanded job data with 10 entries
//   const initialJobs = [
//     {
//       id: 1,
//       title: "Software Engineer",
...
//   // Role suggestions for autocomplete
...
//   // Filter role suggestions based on search term
//   const filteredRoleSuggestions = roleSuggestions.filter(
...
//                   {job.role && (
//                     <p className="text-gray-300 flex items-center text-sm">
//                       <User size={14} className="mr-2 text-purple-400" /> {job.role}
//                     </p>
//                   )}
//                   <button
//                     onClick={() => openApplicationsModal(job)}
...
//                           className="px-4 py-3 w-full border border-gray-700 rounded-md bg-gray-800 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
//                         />
//                         {showRoleSuggestions && roleSearchTerm && (
...
//                               ))
//                             ) : (
//                               <div className="px-4 py-2 text-gray-400">No matching roles found</div>
//                             )}
//                           </div>
//                         )}
//                       </div>
              ...
              >
                ...
                <div className="border-t border-gray-700 p-3 flex justify-between items-center">
                  <div className="flex gap-2">
                    ...
                    <button
                    onClick={()=>openEditModal(job)}
                    className="p-1.5 bg-gray-700 rounded-full text-yellow-400 hover-text-yellow-300 trasnsition-colors"
                      title="Edit">
                        <Edit size={16}/>
                        </button>
                    <button
